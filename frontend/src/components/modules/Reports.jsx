  ...
  return (
    <div className="w-full space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        ...
        <button className="bg-slate-800 hover:bg-slate-700 text-indigo-400 border border-indigo-500/30 px-5 py-2.5 rounded-lg text-sm font-semibold transition-colors flex items-center gap-2" onClick={() => alert("CSV Export will download shortly.")}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl shadow-black/20 lg:col-span-2">
          ...
          <div className="flex flex-col sm:flex-row gap-4 h-48 items-end justify-around pb-6 border-b border-slate-800/50">
            {data.maintenance.map(item => {
              ...
              return (
                <div key={item.asset} className="flex flex-col items-center gap-3 w-full sm:w-auto flex-1 max-w-[120px]">
                  <div className="w-full bg-orange-500/80 rounded-t-sm hover:bg-orange-500 transition-colors relative group" style={{ height: `${height}%` }}>
                  ...
                  <span className="text-xs text-slate-400 font-medium text-center truncate w-full px-2">{item.asset}</span>
                </div>
              );
            })}
          </div>
        ...
        {/* Heatmap Placeholder */}
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl shadow-black/20 lg:col-span-2">
          <h4 className="text-lg font-bold text-white mb-4">Resource Booking Heatmap (Last 30 Days)</h4>
          <div className="w-full bg-slate-800/40 rounded-xl border border-slate-700 h-40 flex items-center justify-center">
            <div className="text-center">
               <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="mx-auto text-indigo-500/50 mb-2"><rect width="18" height="18" x="3" y="3" rx="2"/><path d="M3 9h18"/><path d="M3 15h18"/><path d="M9 3v18"/><path d="M15 3v18"/></svg>
               <p className="text-slate-400 text-sm font-medium">Interactive Heatmap Module Loading...</p>
            </div>
          </div>
        </div>
