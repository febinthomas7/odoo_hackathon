...
const ActiveAuditView = ({ cycle, assets, onBack, onRefresh }) => {
  // Filter assets that belong to the department being audited (match by departmentId)
  const targetAssets = assets.filter(a => a.categoryId != null && (
    a.departmentId === cycle.departmentId ||
    a.department === cycle.department
  ));
  // If no departmentId filter matches, show ALL assets so the auditor can still mark items
  const auditAssets = targetAssets.length > 0 ? targetAssets : assets;
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl shadow-black/20 min-h-[500px]">
      ...
      {isClosed && (
        <div className="mt-8 border-t border-slate-700 pt-8">
          <h3 className="text-xl font-bold text-white mb-4">Auto Discrepancy Report</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-slate-800/40 border border-slate-700 p-4 rounded-lg text-center">
              <span className="block text-2xl font-bold text-emerald-400">{cycle.results.filter(r => r.status === 'Verified').length}</span>
              <span className="text-sm text-slate-400">Assets Verified</span>
            </div>
            <div className="bg-orange-500/10 border border-orange-500/20 p-4 rounded-lg text-center">
              <span className="block text-2xl font-bold text-orange-400">{cycle.results.filter(r => r.status === 'Damaged').length}</span>
              <span className="text-sm text-orange-400/80">Assets Damaged</span>
            </div>
            <div className="bg-red-500/10 border border-red-500/20 p-4 rounded-lg text-center">
              <span className="block text-2xl font-bold text-red-400">{cycle.results.filter(r => r.status === 'Missing').length}</span>
              <span className="text-sm text-red-400/80">Assets Missing</span>
            </div>
          </div>
          <p className="text-sm text-slate-400 mt-4">Missing assets have automatically been marked as "Lost" in the global inventory. Damaged assets are flagged for "Under Maintenance".</p>
        </div>
      )}
    </div>
  );
...
const CreateCycleModal = ({ departments, employees, onClose, onComplete }) => {
  const [formData, setFormData] = useState({ title: '', departmentId: '', startDate: '', endDate: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    await createAuditCycle({
      ...formData,
      auditorIds: auditorId ? [parseInt(auditorId)] : []
    });
    onComplete();
    onClose();
  };
    ...
    <Modal title="Create Audit Cycle" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <select required value={auditorId} onChange={e => setAuditorId(e.target.value)} className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-2.5 focus:ring-1 focus:ring-indigo-500">
