...
const RequestModal = ({ request, assets, employees, onClose, onComplete }) => {
  ...
  const [formData, setFormData] = useState(
    request || { assetId: '', reportedById: '', issue: '', priority: 'Medium', status: 'Pending', technicianId: '', vendorName: '', cost: '', expectedCompletionDate: '' }
  );
        ...
        {!isEdit && (
          <>
            ...
            <div>
              ...
              <select required value={formData.reportedById} onChange={e => setFormData({...formData, reportedById: e.target.value})} className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-2.5 focus:ring-1 focus:ring-indigo-500">
        ...
        {isEdit && (
          <>
            <div className="border-t border-slate-700 pt-4 mt-4">
               <h4 className="text-sm font-bold text-indigo-400 mb-3">Resolution Details</h4>
               <div className="space-y-4">
                 <div>
                   <label className="block text-xs font-medium text-slate-400 mb-1 uppercase tracking-wider">Update Status</label>
                   ...
                   {(formData.status === 'Approved' || formData.status === 'In Progress') && (
                      <p className="text-xs text-orange-400 mt-2 bg-orange-500/10 border border-orange-500/20 p-2 rounded flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><line x1="12" x2="12" y1="9" y2="13"/><line x1="12" x2="12.01" y1="17" y2="17"/></svg>
                        Asset will be marked as "Under Maintenance" (Downtime).
                      </p>
                   )}
                 </div>
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select value={formData.technicianId} onChange={e => setFormData({...formData, technicianId: e.target.value})} className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-2.5 focus:ring-1 focus:ring-indigo-500">
                        <option value="">-- Internal Tech --</option>
                        {employees.map(emp => <option key={emp.id} value={emp.id}>{emp.name}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-slate-400 mb-1 uppercase tracking-wider">Or Vendor Name (External)</label>
                      <input type="text" value={formData.vendorName || ''} onChange={e => setFormData({...formData, vendorName: e.target.value})} placeholder="e.g. Ace Repairs" className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-2.5 focus:ring-1 focus:ring-indigo-500" />
                    </div>
                 </div>

                 <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-medium text-slate-400 mb-1 uppercase tracking-wider">Estimated Cost ($)</label>
                      <input type="number" value={formData.cost || ''} onChange={e => setFormData({...formData, cost: e.target.value})} className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-2.5 focus:ring-1 focus:ring-indigo-500" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-slate-400 mb-1 uppercase tracking-wider">Exp. Completion</label>
                      <input type="date" value={formData.expectedCompletionDate || ''} onChange={e => setFormData({...formData, expectedCompletionDate: e.target.value})} className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-2.5 focus:ring-1 focus:ring-indigo-500 [&::-webkit-calendar-picker-indicator]:filter [&::-webkit-calendar-picker-indicator]:invert" />
                    </div>
                 </div>
               </div>
            </div>
